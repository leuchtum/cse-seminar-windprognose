...
In dieser Arbeit wird die Windgeschwindigkeit für die Messstation in Günzburg prognostiziert. Die übrigen Messstation dienen lediglich der Prognoseverbesserung.
...
\highlight{Random search über Konfigurationen aus Tabelle und daraus optimales NN; optional Grafik über Fehler, Loss, etc.}
...
\highlight{
	\begin{itemize}
		...
		\item Warum sind so Modelle schwierig zu vergleichen? Unterschiedliche Daten in sämtlichen Bereichen!
	\end{itemize}
}
...
\highlight{Wir betrachten Günzburg und binden westliche Stationen ein. Was würde passieren, wenn man stattdessen östliche hernimmt?}
...
Andere Ideen schlagen vollkommen neuartige Ansätze vor. Im Folgenden beschriebende Ideen können in Folgearbeiten näher betrachtet werden.
...
Das Modell, welches \citeauthor{2019_Chen_MultifactorSpatiotemporalCorrelation} in seinem Paper im Jahr 2019 vorstellte \cite{2019_Chen_MultifactorSpatiotemporalCorrelation} ist aufgrund der Verwendung von Messdaten mehrerer Standorte dem hier entwickelten Modell sehr ähnlich. 
\citeauthor{2019_Chen_MultifactorSpatiotemporalCorrelation} verwendete kein RNN, sondern ein CNN mit LSTM Zellen. Ein CNN könnte für den Umgang mit mehreren Input-Standorten geeigneter sein als ein RNN.
\newline
Eine weiterführende Idee davon wäre ein Ensemble-Modell, wie beispielsweise das Modell von \citeauthor{2017_Zhang_HybridMethodShortTerm} \cite{2017_Zhang_HybridMethodShortTerm}, zu entwickeln. Hierbei werden mehrere Prognoseansätze, welche jeweils gute Prognosen bei unterschiedlicher Datenbasis liefern können, kombiniert. 
...
\highlight{
	Einteilen in Punkte für Modellerweiterung und grundsätzlicher Modelländerung.
	Ideen, mit dem man das Modell / die Ergebnisse verbessern kann:
	\begin{enumerate}
		...
		\item SARIMA Modell kann unsere Ergebnisse besser in Kontext setzen: Es kann sein, dass Datenlage in Süddeutschland auch für herkömmliches Modell schwer vorherzusagen ist. Es kann aber auch sein, dass unser Modell nicht genau genug ist.
	\end{enumerate}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                     %
