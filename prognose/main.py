...
if __name__ == '__main__':
    ...
    if load_from_pickle:
        import pickle
        with open("x.pkl", "rb") as loadfile:
            x = pickle.load(loadfile)
        with open("y.pkl", "rb") as loadfile:
            y = pickle.load(loadfile)
    else:
        print("LOAD")
        dwd = MLForecast.sources.DWDStationsHourly(["01886", "02886", "03402", "04887"], ["wind", "air_temperature", "pressure", "sun"])
        df_dwd = dwd.get_data(drop_before=datetime(2016,1,1), drop_after=datetime(2021,1,1))
        cal = MLForecast.sources.CalendricalDataHourly(start=datetime(2016,1,1), end=datetime(2021,1,1))
        df_cal = cal.get_data()
