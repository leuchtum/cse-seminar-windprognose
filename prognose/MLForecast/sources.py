...
def scrap_dwd_urls(baseurl, root_recursive = True):
    ...
    if r.status_code == requests.codes.ok:
        ...
        for suburl in suburls:
            if suburl[-1] == "/":
                final_urls.extend(scrap_dwd_urls(baseurl + suburl, root_recursive=False))
            else:
...
class DWDStationsHourly():
    ...
    def download(self):
        ...
        station_urls = [i for i in self.allurls if any(j in i for j in self.station_ids)]
        self.urls = [i for i in station_urls if any(j in i.split("/") for j in self.observations)]
        
        self.relations = {station_id:[] for station_id in self.station_ids}
        for url in tqdm(self.urls):
            for station_id in self.station_ids:
                if station_id in url:
                    self.relations[station_id].append(get_file(url, self.directory))
                    continue
    ...
    def read_in(self):        
        ...
        recent_dfs = {sid:pd.concat(recent_dfs[sid], axis=1) for sid in self.station_ids}
        historical_dfs = {sid:pd.concat(historical_dfs[sid], axis=1) for sid in self.station_ids}
...
if __name__ == "__main__":
    dwd = DWDStationsHourly(["15444", "04887"], ["wind", "air_temperature", "pressure", "sun"])
