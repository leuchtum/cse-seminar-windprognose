...
def sample_sequences(pre_sampling_x, pre_sampling_y, x_width, y_width, shift):
    ...
    # Rename
    pre_sampling_x = pre_sampling_x.rename(columns={key:f"INPUT_{key}" for key in pre_sampling_x.keys()})
    pre_sampling_y = pre_sampling_y.rename(columns={key:f"LABEL_{key}" for key in pre_sampling_y.keys()})
...
def df_tuple_to_np(df_tuple):
    convert = lambda df_list: np.array([d.values for d in df_list])
...
class DataBaseReport:
    def __init__(self, pre_sampling_x, pre_sampling_y, post_sampling_x, post_sampling_y, shift, split):
        ...
        self.x_stats = pre_sampling_x.describe().loc[["count", "min", "max", "std"]].to_dict()
        self.y_names = pre_sampling_y.keys().to_list()
        self.y_stats = pre_sampling_y.describe().loc[["count", "min", "max", "std"]].to_dict()
